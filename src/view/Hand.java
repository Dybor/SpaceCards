...
public class Hand extends JPanel implements MouseListener, MouseMotionListener {
	...
	private float ratio = (float) 1.25; // ratio de hauteur lors de la selection
										// de la carte

	public Hand() {
		this.setPreferredSize(new Dimension(10 * Board.WIDTH_CARD_HAND,
				(int) (Board.HEIGHT_CARD_HAND * ratio)));
	...
	public void updateCards(ArrayList<DrawableCard> cards) {
		...
		for (DrawableCard card : cards) {
			GraphicCard gCard = new GraphicCard(card.getImage(), x,
					(int) (y + (ratio - 1) * Board.HEIGHT_CARD_HAND),
					Board.WIDTH_CARD_HAND, Board.HEIGHT_CARD_HAND);
	...
	public void mouseExited(MouseEvent e) {
		flagCard = -1; // mise a jour du flag pour le reperage de la carte
						// survolee
	...
	public void upFocusedCard() {
		for (GraphicCard gc : cards) {
			if (cards.indexOf(gc) == flagCard) {
				...
				gc.setY((int) (Board.POSITION_y_HAND + (ratio - 1)
						* Board.HEIGHT_CARD_HAND));
			}
		}
		this.repaint();
	}
