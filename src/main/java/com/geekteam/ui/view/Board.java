...
import com.geekteam.core.controler.INotifyView;
import com.geekteam.core.controler.Observable;
import com.geekteam.core.model.Hand;
...
public class Board extends JFrame implements ActionListener, MouseListener,
		MouseMotionListener, ItemListener, INotifyView, ComponentListener {

	private static final long serialVersionUID = -7137478039137907688L;
	// L'instance de notre objet contr�leur
	private final GameControler controler;
	private final Observable game;
	...
	private final JMenu menu_File = new JMenu("Fichier");
	private final JMenu menu_Game = new JMenu("Jeu");
	private final JMenu menu_Option = new JMenu("Options");

	private final JMenuItem item_new = new JMenuItem("Nouvelle Partie");
	private final JMenuItem item_join = new JMenuItem("Rejoindre une Partie");
	private final JMenuItem item_quit = new JMenuItem("Quitter");

	private final JMenuItem item_test = new JMenuItem("Piocher");

	private final JCheckBoxMenuItem item_fullScreen = new JCheckBoxMenuItem(
			"Plein �cran");

	public static int POSITION_X_HAND = 0; // position de d�part de la main du
	// joueur en x
	public static int POSITION_Y_HAND = 50; // position de d�part de la main du
	// joueur en y
	public static int POSITION_X_BOARD = 0; // position de d�part sur le plateau
	// du joueur en x
	public static int POSITION_Y_BOARD = 0; // position de d�part sur le plateau
	// du joueur en y

	public static int WIDTH_CARD_HAND = 142; // largeur d'une carte dans la main
	// du joueur
	public static int HEIGHT_CARD_HAND = 200;// hauteur d'une carte dans la main
	// du joueur
	public static int WIDTH_CARD_BOARD = 71; // largeur d'une carte sur le
	// plateau du joueur
	public static int HEIGHT_CARD_BOARD = 100;// hauteur d'une carte sur le
	// plateau du joueur
	public static int WIDTH_CARD_ZOOM = 357; // largeur d'une carte zoom�e
	public static int HEIGHT_CARD_ZOOM = 500;// hauteur d'une carte zoom�e

	public static int POSITION_X_ROUND = 0; // position de d�part du tour de jeu
	public static int POSITION_Y_ROUND = 0; // position de d�part du tour de jeu
	...
	public Board(final GameControler controler, final Observable observable) {
	...
	private void fullScreen(final boolean bool) {
	...
	public void refreshAll() {
		...
		pStatus.setVisibleTrueValidateButton(game.getPlayers().get(me)
				.canValidate());
	}
	...
	private void piocher() {
		int me = game.getPlayerViewOwnerIndex();
		com.geekteam.core.model.Hand hand = (Hand) game.getPlayers().get(me)
				.getDrawableHand();
	...
	private void poserCarte() {
		int me = game.getPlayerViewOwnerIndex();
		com.geekteam.core.model.Board board = (com.geekteam.core.model.Board) game
				.getPlayers().get(me).getDrawableBoard();
	...
	public void actionPerformed(final ActionEvent e) {
		if (e.getSource() == item_join) {
			...
			pStatus.setButtonSelected(pStatus.getBrList()
					.indexOf(e.getSource()));
		}
	...
	public void updatePoolPV(final int pv) {

	}

	public void updateHand(final IDrawableHand hand) {
		this.board.updateCards(hand);
	}

	public void updateBoard(final IDrawableBoard board) {
		this.board.updatePlayerBoard(board);
	}

	public void updateOtherBoard(final ArrayList<IDrawableBoard> list) {
		this.board.updateOtherPlayerBoard(list);
	}

	@Override
	public void updateGameRound(final ArrayList<Boolean> bool) {
		gameRound.setGreyRound(bool);

	}

	@Override
	public void sendInformationMessage(final String str) {
	...
	public void mouseMoved(final MouseEvent e) {
		...
		// zoomCard.setCard(new
		// GraphicCard(board.getFocusedCard().getImage(),0,0,Board.WIDTH_CARD_ZOOM,
